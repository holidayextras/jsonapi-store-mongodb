...
const getExtraFilters = request => {
  let extraFilters = {};
  if (request.resourceConfig.filterMapper &&
...
/**
 * Recursive function which runs through a criteria object. 
 * This criteria object can contain strings, arrays or objects, potentially 
 * with mongo expressions sprinkled throughout. After running through the 
 * recursion, the returned criteria will have all it's valid id's which map 
 * to a relationship on the resource converted to a valid mongodb ObjectId.
 *
 * @param {String} key - The key of the criteria attribute
 * @param {*} val - The value of the criteria attribute
...
/**
 * Parses a criteria object so all relationship ids are valid mongodb ObjectIds. 
 *
 * @param {Object} resourceConfig - Configuration details of the resource,
 *                                  defined in it's index.js
 * @param {*} criteria - Generated criteria by the _getSearchCriteria function
 */
const parseCriteria = (resourceConfig, criteria) => {
  console.log(JSON.stringify(criteria));
  const parsedCriteria = {};
  Object.keys(criteria).forEach(key => {
    parsedCriteria[key] = parseRelationshipAttributes(key, criteria[key], resourceConfig);
  });
 ...
 * @param {Object} filter 
 */
parseCriteriaToMongoExpr = (MongoStore, filter) => {
