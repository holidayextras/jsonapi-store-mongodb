...
MongoStore.prototype._getSearchCriteria = request => {
  ...
  let mongoFilter = parseCriteriaToMongoExpr(MongoStore, filter);
  if (request.headers && request.headers.extraParams && request.headers.extraParams.systemFilter) {
    mongoFilter = {...mongoFilter, ...request.headers.extraParams.systemFilter };
  }
  return mongoFilter;
};
