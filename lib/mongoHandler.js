...
MongoStore.prototype.search = function(request, callback) {
  ...
  const parseRelationshipAttributes = (key, val) => {
    if (Array.isArray(val)) {
      return val.map(subVal => parseRelationshipAttributes(key, subVal));
    }
    if (typeof val === 'object') {
      ...
      Object.keys(val).forEach(valKey => {
        ...
        parsedVal[valKey] = parseRelationshipAttributes(keyToCheck, val[valKey]);
      });
      return parsedVal;
    }
  ...
  Object.keys(criteria).forEach(key => {
    criteria[key] = parseRelationshipAttributes(key, criteria[key]);
  });
