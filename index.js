...
MongoStore.prototype._createExamples = function(resourceConfig, callback) {
  var self = this;
  async.each(resourceConfig.examples, function(document, cb) {
    self.create({ params: {} }, document, cb);
  }, function(err) {
    if (err) console.error('error creating sample document:' + err);
    return callback();
  });
};


MongoStore.prototype._initialiseResource = function(resourceConfig, callback) {
  var resourceName = resourceConfig.resource;
  console.log('initialising resource [' + resourceName + ']');
  var collection = this._db.collection(resourceName);
  this._createIndexesForRelationships(collection, resourceConfig.attributes);
  this._createExamples(resourceConfig, callback);
};
...
MongoStore.prototype.initialise = function(resourceConfig) {
  ...
  mongodb.MongoClient.connect(self._config.url).then(function(db) {
    ...
    self._initialiseResource(resourceConfig, function() {
      self.ready = true;
    });
  });
};
